...
class Chwirut : public NIST_Problem{
    public:
        ...
        Eigen::MatrixXd model_function_value(Eigen::VectorXd x, 
            ...
            y(0) = b(0)*exp(-b(1) * x(0)) + b(2)*exp(-b(3) * x(0)) + 
                b(4)*exp(-b(5) * x(0));
            return y;
        }
        ...
        Eigen::MatrixXd model_function_gradient(Eigen::VectorXd x, 
            Eigen::VectorXd b){
                ...
                gradient(0) = exp(-b(1) * x(0));
                gradient(1) = - x(0) * b(0)*exp(-b(1) * x(0));
                gradient(2) = exp(-b(3) * x(0));
                gradient(3) = - x(0) * b(2) * exp(-b(3) * x(0));
                gradient(4) = exp(-b(5) * x(0));
                gradient(5) = - x(0) * b(4) * exp(-b(5) * x(0));
...
int main(){
    std::string file_name = "Lanczos1_dat.txt";
